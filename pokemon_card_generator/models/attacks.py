...
def atk_prediction(pokémon_name, cards_df, rarity):
    ...
    if len(cards_df) == 0:
        raise NameError("Pokémon cards with this Pokémon and rarity shouldn’t exist")
...
def attacks_generator(pokémon_name, cards_df, rarity, ability_presence):
    ...
    df_atk = clean_rarity(cards_df)  # fixed variable overwritten here
    df_atk = clean_damage(cards_df)  # fixed variable overwritten here
